...
def main():
    ...
    objects = [
        obj
        for obj in generate_documentation(obj=lestim, full_name="lestim", level=0)
        if all('[DEPRECATED]' not in line for line in obj.lines)
    ]

    print(f"# Lestim {version} API Reference")
    print()
    if is_dev:
        print("*CAUTION*: this API reference is for the in-development version of lestim.")
...
'''.strip())
    ...
    for obj in objects:
        ...
        if len(obj.full_name.split('.')) > 2:
            ...
            print(f'# (at top-level in the lestim module)')
        print('\n'.join(obj.lines))
        print("```")
